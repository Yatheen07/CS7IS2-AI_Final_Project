...
class Search:
    ...
    def traverse_states(self, frontier, pop_action):
        ...
        number_of_states = 0
        visited = {
            self.rubicsCube.get_configuration_string(): (self.start_state_config, "-1")
        }
            ...
            if currentNode.is_cube_solved():
                ...
                for action, rotation in self.rotations:
                    ...
                    if not config_string in visited:
                        visited[config_string] = (
                            currentNode.get_configuration_string(),
                            action,
                        )
    ...
    def printResults(
        self, path, goal_reached, algorithm, time_elapsed, number_of_states
    ):
    ...
    def bfs(self):
        ...
        goal_reached, visited, currentNode, number_of_states = self.traverse_states(
            frontier, pop_action
        )
        # self.printResults(
        #     path=self._trace_path(currentNode, visited),
        ...
        return (
            goal_reached,
            self._trace_path(currentNode, visited),
            time.time() - start,
            number_of_states,
        )
    ...
    def iddfs(self):
        ...
        goal_reached = False
        visited = {
            self.cube.get_configuration_string(): (self.start_state_config, "-1")
        }
        ...
        while cost_limit <= self.depth:
            ...
            while len(frontier) != 0:
                ...
                if currentState.cost + 1 <= cost_limit:
                    ...
                    if currentState.parent is not None and (
                        self.commonParent(new.cube, currentState)
                        or self.containsChild(new.cube, frontier)
                    ):
                        continue
                    frontier.append(new)
                    branch += 1
        ...
        while not queue.isEmpty():
            ...
            if currentNode.state.is_cube_solved():
                ...
                for action, rotation in self.rotations:
                    ...
                    if not config_string in visited:
                        visited[config_string] = (
                            currentNode.state.get_configuration_string(),
                            action,
                        )
