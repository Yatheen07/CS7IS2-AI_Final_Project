...
class Agent:
    ...
    def register_patterns(self):
        ...
        for action in self.actions:
            ...
            for action_ in self.actions:
                self.QV[(s_.__hash__(),
                         action_)] = -10 if action_ != action else 10
        ...
        for s in self.one_away:
            for action in self.actions:
                ...
                for action_ in self.actions:
                    self.QV[(s_.__hash__(),
                             action_)] = -6 if action_ != action else 6
        ...
        for s in self.two_away:
            for action in self.actions:
                ...
                for action_ in self.actions:
                    self.QV[(s_.__hash__(),
                             action_)] = -5 if action_ != action else 5
        ...
        for s in self.three_away:
            for action in self.actions:
                ...
                for action_ in self.actions:
                    self.QV[(s_.__hash__(),
                             action_)] = -4 if action_ != action else 4
        ...
        for s in self.four_away:
            for action in self.actions:
                ...
                for action_ in self.actions:
                    self.QV[(s_.__hash__(),
                             action_)] = -3 if action_ != action else 3
        ...
        for s in self.five_away:
            for action in self.actions:
                ...
                for action_ in self.actions:
                    self.QV[(s_.__hash__(),
                             action_)] = -1 if action_ != action else 1
        ...
        for i in range(episodes):
            ...
            for action in self.actions:
                if not (self.curr_state.__hash__(), action) in self.QV.keys():
                    ...
                    break
                if not saved_rewards:
                    self.R[self.curr_state.__hash__()].append(
                        self.reward(self.curr_state, action))
            ...
            if follow_policy > epsilon:
                ...
                #     #       " from curr state is " +
                #     #       str(self.QV[(self.curr_state.__hash__(), action)]))
                best_action = None
                best_QV = -100000000
                for action in self.actions:
                    if self.QV[(
                            self.curr_state.__hash__(), action
                ...
                #self.QV[(self.curr_state.__hash__(), best_action)] = best_QV + self.alpha*(reward +\
                ...
                for action in self.actions:
                    ...
                    self.QV[(self.curr_state.__hash__(), action)] = curr_QV + self.alpha*(reward +\
                                                         (discount**vc)*max_reward - curr_QV)
                # #print("new q value for " + best_action + " action is " +
                #       str(self.QV[(self.curr_state.__hash__(), best_action)]))
    ...
    def Play(self):
        ...
        for i in range(30):
            best_action = None
            best_QV = -100000000
            if not (self.curr_state.__hash__(),
                    self.actions[0]) in self.QV.keys():
                best_action = random.choice(self.actions)
